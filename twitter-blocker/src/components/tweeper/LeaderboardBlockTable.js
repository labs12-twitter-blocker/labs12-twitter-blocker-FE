...
class LeaderboardBlockTable extends React.Component {
  state = {
    order: 'asc',
    orderBy: 'list_points',
    data: [],
  ...
  componentDidUpdate() {
    if (this.props.blockLists !== null  && this.state.blockListRan === false && this.state.twitter_user_id !== "" ) {
      this.getListRowBuilder(this.props.blockLists);
    }
  };
  ...
  render() {
    if (this.props.blockLists === null || this.props.blockLists.length === 0 || this.state.twitter_user_id === "" ) {
...
export default connect(
  mapStateToProps,
  { getBlockListPoints, addUserVote }
)(styledComponent);
